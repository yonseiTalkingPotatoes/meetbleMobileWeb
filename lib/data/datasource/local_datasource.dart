...
List<DateRangeModel> dateShortcutList = [
  DateRangeModel(
      rangeName: "이번주",
      start: DateTime.now(),
  ...
  DateRangeModel(
      ...
      start: DateTime.now().add(Duration(
          days: DateTime.now().weekday == 7 ? 0 : 6 - DateTime.now().weekday)),
      days: DateTime.now().weekday == 7 ? 1 : 2),
  DateRangeModel(
    rangeName: "이번달",
    start: DateTime.now(),
    days: (DateTime.now().month == 12
...
List<TimeRangeModel> timeShortcutList = [
  TimeRangeModel(
    rangeName: '전체',
    start: 0,
    duration: 24,
